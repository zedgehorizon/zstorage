...
export async function uploadFilesRequest(filesToUpload: FormData, nativeAuthToken: string, customApiVersion: string = API_VERSION) {
  try {
    ...
    if (error?.response.data.statusCode === 403) {
      ...
      toast("You have exceeded your usage limit. A paid plan or plan upgrade is required to continue.");
    } else toast.error("Error uploading files to your data bunker: " + `${error ? error.message + ". " + error?.response?.data.message : ""}`);
    return error;
  }
}

export async function uploadFilesRequestWalrus(filesToUpload: FormData, nativeAuthToken: string) {
  try {
    const response = await axios.post(`${import.meta.env.VITE_ENV_BACKEND_API}/walrus/upload`, filesToUpload, {
      headers: {
        "authorization": `Bearer ${nativeAuthToken}`,
      },
    });
...
/*
// KEEPING this for reference, but we are using the new uploadFilesRequestWalrus function instead
export async function uploadFilesRequestSUIWalrusLegacy(filesToUpload: FormData, nativeAuthToken: string) {
  try {
...
export async function getUserAvailableSpaceAndBandwidth(nativeAuthToken: string) {
  try {
    ...
    let accountTier = response.data.accountTier;
